...
const allMatchingProjects = [
    ...configuration.projects.reduce((projects, project) => {
        glob_1.default.sync(project).forEach((p) => {
            projects.add(p);
        });
        return projects;
    }, new Set()),
...
const register = (0, register_1.default)(factoryParams);
const runners = (0, run_1.default)(factoryParams);
(0, jest_each_1.default)(projects).describe(projectTitle, (project) => {
    try {
        ...
        const suites = project.suites.filter((suite) => suite.tests.length && new RegExp(configuration.filter).test(suite.name));
        if (suites.length) {
